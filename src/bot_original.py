...
from db import (
    ...
    create_value_snapshot, get_completed_session_count,
...
from values import (
    ...
    ALL_VALUES, VALUE_DEFINITIONS, generate_session_insight
)
...
# Anthropic client is now in client.py
...
async def process_session_message(
    ...
    try:
        ...
        for belief in extraction.beliefs:
            ...
            # Track for session close display (with themes)
            session.extracted_beliefs.append({
                'statement': belief.statement,
                'themes': [v.name for v in belief.values] if belief.values else []
            })
...
async def generate_session_response(session: SessionState, user_message: str) -> str:
    ...
    try:
        ...
        return "Sorry, I'm being slow right now. Can you try saying that again?"
    except anthropic.APIError as e:
        logger.error(f"API error generating response: {e}")
...
async def themes_command(interaction: discord.Interaction):
    """Display themes Kodak has noticed in the user's reflections."""
...
# Keep /values as alias for backwards compatibility
...
async def values_command(interaction: discord.Interaction):
    """Alias for /themes."""
    await themes_command(interaction)
...
async def themes_history_command(interaction: discord.Interaction):
    """Show how themes have shifted over time."""
    await interaction.response.defer(ephemeral=True)
    ...
    if not month_ago_profile:
        await interaction.followup.send(
            "Not enough history yet to show shifts.\n\n"
            "Keep journaling — after a month I'll be able to show how your themes are shifting.",
            ephemeral=True
        )
        return
    ...
    if not change_narrative:
        await interaction.followup.send(
            "Your themes have been pretty stable over the past month.\n\n"
            "No major shifts in what you keep coming back to.",
            ephemeral=True
        )
        return
...
# Keep /values-history as alias for backwards compatibility
...
class ShareValuesView(discord.ui.View):
    ...
    async def generate_file(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.followup.send(
            f"Here's your themes export! Share this file with someone who uses Kodak.\n"
            f"They can use `/compare-file` to explore how your themes align and differ.",
            file=file,
            ephemeral=True
        )
...
async def share_themes_command(interaction: discord.Interaction):
    """Export themes as shareable JSON with privacy controls."""
    await interaction.response.defer(ephemeral=True)
    ...
    if not top_values or all(v.normalized_score == 0 for v in top_values):
        await interaction.followup.send(
            ...
            "Keep journaling and I'll notice more patterns over time.",
            ephemeral=True
        )
        return
    ...
    # Show current top themes
    top_names = [f"**{v.display_name}**" for v in top_values[:3] if v.normalized_score > 0.2]

    await interaction.followup.send(
        "**Share Your Themes**\n\n"
        f"Themes I've noticed: {', '.join(top_names)}\n\n"
        "Choose what to include in your export:\n"
        "• **Themes** — what you keep coming back to (default: all)\n"
        "• **Beliefs** — sample statements from your reflections (optional)\n"
        "• **Display name** — how you appear to the recipient\n\n"
        "Then click **Generate file** to create your shareable profile.",
        view=view,
        ephemeral=True
    )
    logger.info(f"User {user_id} started share-themes flow")


# Keep /share-values as alias for backwards compatibility
...
async def compare_file_command(interaction: discord.Interaction, file: discord.Attachment):
    """Load and compare themes with a shared export."""
    await interaction.response.defer(ephemeral=True)
    ...
    try:
        ...
        if not imported:
            await interaction.followup.send(
                "Couldn't read that file. Make sure it's a Kodak export.\n\n"
                "Ask your friend to use `/share-themes` to create a valid export.",
                ephemeral=True
            )
            return
        ...
        if not top_values or all(v.normalized_score == 0 for v in top_values):
            await interaction.followup.send(
                "You need more conversations first before comparing.\n\n"
        ...
        logger.info(f"User {user_id} compared themes with {imported.display_name}")
...
async def summary_command(
    ...
    # Import here to avoid circular imports
    from summaries import create_weekly_summary, format_date_range
...
async def help_command(interaction: discord.Interaction):
    """Show help information. Essential commands by default, full list with 'all'."""

    # Essential commands (what most users need)
    embed = discord.Embed(
        title="Kodak",
        description="Just message me anytime to start journaling.",
        color=0x7289da
    )
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`/themes` — See patterns I've noticed\n"
            "`/summary week` — Your weekly digest\n"
            "`/schedule [time]` — Set daily check-in time\n"
            "`/map` — See your beliefs by topic"
        ),
        inline=False
    )

    # Add button view for "See all commands"
    view = HelpExpandView()
    await interaction.response.send_message(embed=embed, view=view, ephemeral=True)


class HelpExpandView(discord.ui.View):
    """View with a button to expand to full command list."""

    def __init__(self):
        super().__init__(timeout=120)

    @discord.ui.button(label="See all commands", style=discord.ButtonStyle.secondary)
    async def expand_help(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(
            title="All Commands",
            color=0x7289da
        )
        ...
        embed.add_field(
            name="Themes & Summaries",
            value=(
                "`/themes` — See patterns I've noticed\n"
                "`/themes-history` — How themes shifted over time\n"
                "`/summary week` — Weekly digest\n"
                "`/summaries` — View past summaries\n"
                "`/share-themes` — Export your themes to share\n"
        ...
        embed.add_field(
            name="Edit Beliefs",
            value=(
                "`/confidence [id] [level]` — Update confidence\n"
                "`/mark [id] [importance]` — Set importance\n"
